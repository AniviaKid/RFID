	...
	if(password.IsEmpty()) MessageBox("请输入密钥");
		...
		if(cur_page==-1) MessageBox("请选择扇区");
			...
			if(return_state!=0){
				...
				MessageBox("读块失败,状态码为"+tmp);
				return;
			}

			//写内容
			CString tmp;
			if(cur_block==0||cur_block==1||cur_block==2){ //块0 块1 块2
